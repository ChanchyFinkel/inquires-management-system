...
export class InquiryStateService {
  ...
  updateStatus(id: number, statusId: number): Observable<InquiryResponse> {
    ...
    return this.inquiryService.updateStatus(id, statusId).pipe(
      switchMap((updated) => {
        const activeStatusFilter = this.filterState.value.statusId;
        if (activeStatusFilter != null && activeStatusFilter !== updated.statusId) {
          // The item no longer matches the active status filter, so refetch the current
          // page from the server instead of patching it in place, otherwise it would keep
          // showing under a filter it no longer belongs to.
          this.refresh();
        } else {
          this.patchViewState({
